...
# pytorch 기반 새로만들기
class PolicyLearner:
    def __init__(self, stock_code, chart_data, training_data=None, min_trading_unit=1, max_traiding_unit=2,
        ...
        self.agent = Agent(environment=self.environment,
                           max_trading_unit=max_traiding_unit,
                           min_trading_unit=min_trading_unit,
                           delayed_reward_threshold=delayed_reward_threshold
                           )
    ...
    def fit(self, num_epoch=1000, max_memory=60, balance=1000000, discount_factor=0, start_epsilon=.5, learning=True):
        logger.info(
            f"LR:{self.policy_network.lr}, DF:{discount_factor} TU:[{self.agent.min_trading_unit}, {self.agent.max_trading_unit}], DRT:{self.agent.delayed_reward_threshold}")
        ...
        for epoch in range(num_epoch):
            ...
            # 학습 진행할 수록 탐험비율 감소
            if learning:
                epsilon = start_epsilon * (1. - float(epoch) / (num_epoch - 1))
            else:
                epsilon = 0

            while True:
                # 샘플 생성
                next_smaple = self._build_sample()
                if next_smaple is None:
                    break

                # 정책 신경망 또는 탐험에 의한 행동 결정
                action, confidence, exploration = self.agent.decide_action(self.policy_network, self.sample, epsilon)

                # 결정한 행동을 수행하고 즉시보상 & 지연보상 획득
                immediate_reward, delayed_reward = self.agent.act(action, confidence)

                # 행동 및 행동에 대한 결과를 기억
                memory_sample.append(next_smaple)
                ...
                if learning and delayed_reward != 0:
                    ...
                    batch_size = 0
                    # 에포크 관련 정보 가시화
                    num_epoch_digit= len(str(num_epoch))
                    epoch_str = str(epoch+1).rjust(num_epoch_digit,'0')
                    # 미구현
                    ...
                    logger.info("[Epoch %s/%s]\tEpsilon:%.4f\t#Expl.:%d%d\t"
                        ...
                        epoch_str, num_epoch, epsilon, exploration_cnt,itr_cnt,
                        self.agent.num_buy, self.agent.num_sell, self.agent.num_hold,
                        self.agent.num_stocks, locale.currency(self.agent.portfolio_value, grouping=True),
    ...
    def trade(self, model_path=None, balance=2000000):
        ...
        self.fit(balance=balance, num_epoch=1, learning=False)
